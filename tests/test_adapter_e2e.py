...
def _has_project_dir(adapter) -> bool:
    """检查适配器引用的项目目录是否存在"""
    # 从 adapter 模块的全局变量中提取项目路径
    module_name = adapter.__class__.__module__
    mod = sys.modules.get(module_name)
    if mod is None:
        return False
    # 检查模块中定义的 *_PROJECT_DIR 或 *_CLI 变量
    for attr_name in dir(mod):
        if attr_name.endswith("_PROJECT_DIR") or attr_name.endswith("_CLI"):
            val = getattr(mod, attr_name)
            if isinstance(val, str) and ("G:" in val or "/" in val):
                path = Path(val)
                if attr_name.endswith("_CLI"):
                    return path.exists()
                return path.is_dir()
    return True  # 无法判断时默认可用
...
class TestAdapterRegistration:
    ...
    def test_all_10_adapters_registered(self):
        ...
        assert len(registry) == 10, f"期望10个适配器，实际{len(registry)}个"

    def test_all_ids_present(self):
        registry = get_registry()
        expected_ids = {"duan", "yan", "moyan", "xinyu", "zhixing",
                       "yanlv", "yanzhi", "traeyan", "mingdao", "hanyu"}
        actual_ids = set(registry.language_ids)
        assert expected_ids == actual_ids, f"缺失: {expected_ids - actual_ids}, 多余: {actual_ids - expected_ids}"
...
class TestEndToEndExecution:
    ...
    def test_yan_run_file(self):
        ...
        with tempfile.NamedTemporaryFile(
            suffix=".yan", mode="w", encoding="utf-8", delete=False,
        ) as f:
            f.write(code)
            tmp = f.name
        try:
    ...
    def test_moyan_run_file(self):
        ...
        with tempfile.NamedTemporaryFile(
            suffix=".moyan", mode="w", encoding="utf-8", delete=False,
        ) as f:
            f.write(code)
            tmp = f.name
        try:
