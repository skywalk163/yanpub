...
评分维度（总分 100）：
1. 基础完整度（0-30）：adapter.py + adapter.yaml 存在、类定义正确、可实例化
2. 元数据质量（0-20）：YAML 字段完整、版本号合法、扩展名正确、颜色有效
3. 示例丰富度（0-25）：examples/ 目录存在、示例数量、front matter 质量
4. 文档覆盖（0-15）：README/CONTRIBUTING、关键字文档、描述完整性
5. 功能验证（0-10）：eval/run/repl 可用性、关键字加载、健康检查通过
...
class QualityChecker:
    ...
    def _check_basic(self, adapter_dir: Path, lang_id: str) -> DimensionScore:
        """基础完整度检查（0-30）"""
        dim = DimensionScore(name="基础完整度", max_score=30, score=0)
    ...
    def _check_metadata(self, adapter_dir: Path, lang_id: str) -> DimensionScore:
        ...
        if found == len(required_fields):
            ...
            dim.score += int(8 * found / len(required_fields))
    ...
    def _check_examples(self, adapter_dir: Path, lang_id: str) -> DimensionScore:
        """示例丰富度检查（0-25）"""
        dim = DimensionScore(name="示例丰富度", max_score=25, score=0)
        ...
        if count > 0 and fm_count == count:
            dim.score += 10
            dim.details.append("所有示例都有完整的 front matter")
        elif fm_count > 0:
            ratio = fm_count / count
            dim.score += int(10 * ratio)
    ...
    def _check_docs(self, adapter_dir: Path, lang_id: str) -> DimensionScore:
        ...
        if readme_path.exists():
            dim.score += 5
            dim.details.append("README.md 存在")
        else:
        ...
        if keywords_json.exists():
            try:
                ...
                if isinstance(data, list) and len(data) > 0:
                    # 检查是否包含描述
                    if isinstance(data[0], dict) and data[0].get("description"):
                        has_keyword_docs = True
        ...
        if has_keyword_docs:
            dim.score += 4
            dim.details.append("关键字文档存在")
        else:
            dim.suggestions.append("建议完善关键字文档（keywords.json 含描述字段）")
        ...
        if yaml_path.exists():
            try:
                ...
                if desc and len(desc) >= 10:
                    dim.score += 3
                    dim.details.append("语言描述完整")
                elif desc:
                    dim.score += 1
                    dim.suggestions.append("语言描述过短，建议补充到 10 字以上")
                else:
    ...
    def _check_functionality(self, adapter_dir: Path, lang_id: str) -> DimensionScore:
        """功能验证检查（0-10）"""
        dim = DimensionScore(name="功能验证", max_score=10, score=0)
        ...
        try:
            ...
            if kw_count > 0:
                dim.score += 4
                dim.details.append(f"关键字加载成功: {kw_count} 个")
            else:
        ...
        try:
            ...
            if caps:
                ...
                dim.details.append(f"声明的能力: {', '.join(cap_list) if cap_list else '（无）'}")
            else:
                dim.score += 1
        ...
        # eval 可用 (3)
        try:
            has_eval = bool(adapter.eval_command) if hasattr(adapter, "eval_command") else False
            if has_eval:
                dim.score += 3
    ...
    def generate_html(self, reports: list[QualityReport], output_path: Path) -> Path:
        ...
        for r in reports:
            ...
            <div class="card">
                ...
                {"<div style='margin-top:8px;color:#f39c12;font-size:13px'>建议: " + "; ".join(s for d in r.dimensions for s in d.suggestions[:3]) + "</div>" if any(d.suggestions for d in r.dimensions) else ""}
            </div>"""
...
<body>
<h1>适配器质量报告</h1>
<p style="text-align:center;color:#888">{len(reports)} 个适配器 | 平均分 {sum(r.total_score for r in reports)/len(reports):.1f}/{reports[0].max_score if reports else 100}</p>
{rows}
</body>
</html>"""
