...
访问 http://localhost:8080 即可使用 Playground。
...
### v1.9.0 (架构卫生第三轮)
- 9 个 600+ 行文件科学拆分（adapter/ai_assist/navigator+refactor/collab/wasm/versionset/signing/bench_viz）
- 提取 `_ident_utils.py` 消除 navigator.py 与 refactor.py 重复代码
- 最大源文件从 862 行降至 830 行，拆分保留向后兼容

### v1.8.0 (架构卫生第二轮)
- 7 个 800+ 行文件科学拆分（syntax_matrix/server/profiler/sandbox/debugger/site_builder/adapter_test）
- 800+ 行文件从 7 个降至 0 个，所有主文件 ≤490 行
- 拆分保留向后兼容（re-export），公共 API 不变

### v1.7.0 (新增华语适配器)
- 新增华语（华码/Hua）语言适配器（69 关键字，69 个从词法.py动态加载）
- 6 个示例文件 + 7 个 Playground 模板
- 语法对比矩阵新增华语 15 个概念条目
